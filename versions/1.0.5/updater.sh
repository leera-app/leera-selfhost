...
#   /data/update/qa-runner-result.json
#                                outcome of the last test runner switch
#
# A request is an update unless its `kind` says otherwise. The one other kind,
# `qa_runner`, turns the built-in browser test runner on or off through
# `install.sh --qa-runner`; its token is validated here and handed over in the
...
QA_RESULT_FILE="$UPDATE_DIR/qa-runner-result.json"
...
STEPS_TEMPLATE='[
  ...
  {"key":"migrate","label":"Updating the database and stored files","status":"pending"},
...
qa_runner_finish() {
  local status="$1" message="$2"
  jq -n --arg id "$JOB_ID" --arg status "$status" --arg message "$message" \
    '{id: $id, status: $status, message: $message, finished_at: (now | floor)}' \
    > "$QA_RESULT_FILE.tmp" && mv -f "$QA_RESULT_FILE.tmp" "$QA_RESULT_FILE"
  rm -f "$CURRENT_FILE"
  log "test runner switch $status: $message"
}

# Start or remove the built-in test runner. Quick, and the API stays up, so
# there is no progress page: the Test runners page polls the API for the result.
run_qa_runner() {
  JOB_ID="$(jq -r '.id // ""' "$CURRENT_FILE")"
  local enabled token mode
  enabled="$(jq -r '.enabled // false' "$CURRENT_FILE")"
  token="$(jq -r '.token // ""' "$CURRENT_FILE")"

  if [ "$enabled" = "true" ]; then
    mode=on
    # The one value in this request that reaches install.sh. A pool token is a
    # fixed prefix and URL-safe characters; anything else is refused, not passed on.
    if [[ ! "$token" =~ ^pm_run_[A-Za-z0-9_-]+$ ]]; then
      qa_runner_finish failed "The request carried no usable runner token."
      return 0
    fi
  else
    mode=off
  ...
  log "switching the test runner $mode"
  STATE_FILE=""
  LOG_FILE="$(mktemp)"
  STATUS="running"; MESSAGE=""; ERROR_MESSAGE=""; EXIT_CODE=1
  STEPS_JSON="$STEPS_TEMPLATE"

  local env_args=("LEERA_HOME=$INSTALL_DIR" "LEERA_QA_RUNNER_TOKEN_NEW=$token")
  local project
  project="$(stack_project_name)"
  [ -n "$project" ] && env_args+=("COMPOSE_PROJECT_NAME=$project")
  ...
  while IFS= read -r line; do
    ...
    cd "$INSTALL_DIR" || exit 1
    env "${env_args[@]}" bash ./install.sh --qa-runner "$mode" --json-progress 2>&1
    printf '__EXIT__ %s\n' "$?"
  )

  if [ "${EXIT_CODE:-1}" = "0" ]; then
    if [ "$mode" = "on" ]; then
      qa_runner_finish success "The test runner is running."
    else
      qa_runner_finish success "The test runner is stopped."
    fi
  else
    qa_runner_finish failed "${ERROR_MESSAGE:-The test runner could not be switched $mode. Check the update service log on the server.}"
  fi
  rm -f "$LOG_FILE"
}
...
# A test runner switch changes one container and a line of .env, so it is
# reported the same way and simply tried again.
if [ -f "$CURRENT_FILE" ] && [ "$(jq -r '.kind // "update"' "$CURRENT_FILE" 2>/dev/null)" = "qa_runner" ]; then
  JOB_ID="$(jq -r '.id // ""' "$CURRENT_FILE")"
  qa_runner_finish failed "The update service restarted while switching the test runner. Try again."
...
while true; do
  if [ -f "$REQUEST_FILE" ]; then
    ...
    if mv -f "$REQUEST_FILE" "$CURRENT_FILE" 2>/dev/null; then
      if [ "$(jq -r '.kind // "update"' "$CURRENT_FILE" 2>/dev/null)" = "qa_runner" ]; then
        run_qa_runner
      else
        run_update
      fi
